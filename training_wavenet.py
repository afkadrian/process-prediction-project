...
def iterate_over_traces(
    ...
    for mini_batch in iter(data_loader):
        ...
        if model.architecture == "GPT":
            ...
            activity_correct += (predicted_activities == a_s_t).sum().item()
            activity_total += a_s_t.numel()

        elif model.architecture == "BERT":
            categorical_loss = categorical_criterion(
                prediction[0][:, model.mlm.masked_indexes, :].transpose(2, 1),
                a_s_t[:, model.mlm.masked_indexes],
            )

            # Calculate activity accuracy for BERT
            predicted_activities = torch.argmax(
                prediction[0][:, model.mlm.masked_indexes, :], dim=2
            )
            activity_correct += (
                (predicted_activities == a_s_t[:, model.mlm.masked_indexes])
                .sum()
                .item()
            )
            activity_total += a_s_t[:, model.mlm.masked_indexes].numel()
        ...
        if len(prediction) > 1:
            ...
            if model.architecture == "GPT":
                ...
                # Calculate time MSE
                time_squared_errors.extend(
                    (prediction[1] - t_s_t).pow(2).flatten().tolist()
                )

            elif model.architecture == "BERT":
                regression_loss = regression_criterion(
                    prediction[1][:, model.mlm.masked_indexes, :],
                    t_s_t[:, model.mlm.masked_indexes, :],
                )

                # Calculate time MSE for BERT
                time_squared_errors.extend(
                    (
                        prediction[1][:, model.mlm.masked_indexes, :]
                        - t_s_t[:, model.mlm.masked_indexes, :]
    ...
    activity_accuracy = activity_correct / activity_total if activity_total > 0 else 0
    time_mse = np.mean(time_squared_errors) if time_squared_errors else 0
    ...
    if len(prediction) > 1:
        ...
        return (summa_categorical_loss.item() / steps, activity_accuracy, time_mse)
...
def main(args, dt_object):
    ...
    for log_name in logs:
        ...
        augmented_log = data_preprocessing.create_transformer_augmentation(
            split_log,
        ...
        attributes_meta = {
            0: {
                ...
                "vocabulary_size": augmented_log["vocabulary_size"],
            },
        ...
        for i in range(amount_of_fix_masked_positions):
            index_to_append = (
                nb_unmasked_positions_between_masked_ones
                + i * nb_unmasked_positions_between_masked_ones
            )
            if index_to_append < max_length:
                fix_masks.append(index_to_append)
        ...
        for e in range(args.nb_epoch):
            ...
            with torch.no_grad():
                ...
                if args.architecture == "BERT":
                    ...
                    model.mlm.fix_masks = torch.tensor(
                        fix_masks, device=next(model.parameters()).device
                    )
                    validation_loss_fix_masks = iterate_over_traces(
                        log_with_traces=augmented_log,
                        model=model,
                        device=args.device,
                        categorical_criterion=categorical_criterion,
            ...
            if args.early_stopping:
                ...
                if args.architecture == "BERT":
                    if len(total_validation_losses_fix_masks) > early_stopping_var:
                        if np.all(
                            np.array(total_validation_losses_fix_masks)[
                                -(early_stopping_var - 1) :
                            ]
                            > np.array(total_validation_losses_fix_masks)[
                                -early_stopping_var
                            ]
                    ...
                    if len(total_validation_losses) > early_stopping_var:
                        if np.all(
                            np.array(total_validation_losses)[
                                -(early_stopping_var - 1) :
                            ]
            ...
            if args.architecture == "BERT":
                if total_validation_loss_fix_masks < min_loss_threshold:
                    model_to_save["model_state_dict"] = copy.deepcopy(
                        model.state_dict()
                    )
                    model_to_save["optimizer_state_dict"] = copy.deepcopy(
                        optimizer.state_dict()
                    )
                    model_to_save["loss"] = copy.deepcopy(
                        total_validation_loss_fix_masks
                    )
