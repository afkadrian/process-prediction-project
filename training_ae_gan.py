...
def seq_ae_predict(
    ...
    prediction = ()
    use_teacher_forcing = (
        True if random.random() < seq_ae_teacher_forcing_ratio else False
    )
    ...
    if use_teacher_forcing:
        ...
        input_sos = (
            model_input_y[0][:, 0, :].unsqueeze(-1),
            model_input_y[1][:, 0, :].unsqueeze(-1),
        )
        input_position = input_sos

        for i in range(model_input_y[0].size(1)):
            inter_position, decoder_hidden = model.decoder.cell(
                model.decoder.value_embedding(input_position), decoder_hidden
            )
            ...
            if i == 0:
                ...
                a_pred = torch.cat((prediction[0], output_position[0]), dim=1)
                t_pred = torch.cat((prediction[1], output_position[1]), dim=1)
                prediction = (a_pred, t_pred)

            a_inp = output_position[0].detach()
            t_inp = output_position[1].detach()

            input_position = (
                utils.generate(
                    a_inp, temperature=temperature, top_k=top_k, sample=sample
                ),
                t_inp,
            )
...
def iterate_over_prefixes(
    ...
    if subset == "training":
        optimizer_generator = optimizer[0]
        optimizer_discriminator = optimizer[1]
    ...
    if not to_wrap_into_torch_dataset:
        for prefix in log_with_prefixes[subset + "_prefixes_and_suffixes"][
            ...
            times_prefixes = log_with_prefixes[subset + "_prefixes_and_suffixes"][
                "times"
            ]["prefixes"][prefix]
            activities_suffixes_input = log_with_prefixes[
                subset + "_prefixes_and_suffixes"
            ]["activities"]["suffixes"]["input"][prefix]
            times_suffixes_input = log_with_prefixes[subset + "_prefixes_and_suffixes"][
                "times"
            ]["suffixes"]["input"][prefix]
            activities_suffixes_target = log_with_prefixes[
                subset + "_prefixes_and_suffixes"
            ]["activities"]["suffixes"]["target"][prefix]
            times_suffixes_target = log_with_prefixes[
                subset + "_prefixes_and_suffixes"
            ]["times"]["suffixes"]["target"][prefix]
            ...
            for i in range(nb_iterations):
                if device == "GPU":
                    ...
                    times_prefixes_batch = (
                        times_prefixes[i * batch_size : i * batch_size + batch_size, :]
                        .unsqueeze(2)
                        .cuda()
                    )
                    activities_suffixes_input_batch = (
                        activities_suffixes_input[
                            i * batch_size : i * batch_size + batch_size, :
                        ]
                        .unsqueeze(2)
                        .cuda()
                    )
                    times_suffixes_input_batch = (
                        times_suffixes_input[
                            i * batch_size : i * batch_size + batch_size, :
                        ]
                        .unsqueeze(2)
                        .cuda()
                    )
                    activities_suffixes_target_batch = (
                    ...
                    activities_prefixes_batch = activities_prefixes[
                        i * batch_size : i * batch_size + batch_size, :
                    ].unsqueeze(2)
                    times_prefixes_batch = times_prefixes[
                        i * batch_size : i * batch_size + batch_size, :
                    ].unsqueeze(2)
                    activities_suffixes_input_batch = activities_suffixes_input[
                        i * batch_size : i * batch_size + batch_size, :
                    ].unsqueeze(2)
                    times_suffixes_input_batch = times_suffixes_input[
                        i * batch_size : i * batch_size + batch_size, :
                    ].unsqueeze(2)
                    activities_suffixes_target_batch = activities_suffixes_target[
                        i * batch_size : i * batch_size + batch_size, :
                    ].long()
                    times_suffixes_target_batch = times_suffixes_target[
                        i * batch_size : i * batch_size + batch_size, :
                    ].unsqueeze(2)

                if subset == "validation":
                    prediction = seq_ae_predict(
                        seq_ae_teacher_forcing_ratio=1.0,
                        model=generator,
                    ...
                    prediction = seq_ae_predict(
                        seq_ae_teacher_forcing_ratio=args.seq_ae_teacher_forcing_ratio,
                        model=generator,
                        model_input_x=(activities_prefixes_batch, times_prefixes_batch),
                        model_input_y=(
                            activities_suffixes_input_batch,
                ...
                if subset == "training":
                    ...
                    soft_sampled_prediction_categories_for_discriminator = (
                        torch.nn.functional.gumbel_softmax(
                            logits=prediction[0].detach(), tau=t, hard=False
                        )
                    )
                    soft_sampled_target_categories_for_discriminator = (
                        utils.label_smooth_gumbel_sampling(
                    ...
                    loss_real = adversarial_criterion(
                        prediction_real,
                        torch.ones(
                    ...
                    soft_sampled_prediction_categories_for_generator = (
                        torch.nn.functional.gumbel_softmax(
                            logits=prediction[0], tau=t, hard=False
                        )
                    )
                    prediction_fake = discriminator(
                        (
                ...
                if len(prediction) > 1:
                    ...
                    if subset == "training":
                        (
                            categorical_loss
                            + lagrange_a * regression_loss
                            + lagrange_b * adversarial_loss
                        ).backward()
                    ...
                    if subset == "training":
                        (categorical_loss + lagrange_b * adversarial_loss).backward()
        ...
        for prefix in prefixes:
            ...
            for mini_batch in iter(data_loader):
                ...
                if subset == "training":
                    ...
                    soft_sampled_target_categories_for_discriminator = (
                        utils.label_smooth_gumbel_sampling(
                            t=torch.nn.functional.one_hot(a_s_t).float()
                        )
                    )
                    optimizer_discriminator.zero_grad()
                    prediction_fake = discriminator(
                    ...
                    prediction_real = discriminator(
                        (soft_sampled_target_categories_for_discriminator, t_s_t)
                    )
                    loss_real = adversarial_criterion(
                        prediction_real,
                        torch.ones(
...
def main(args, dt_object):
    ...
    for log_name in logs:
        if args.device == "GPU":
            print(
                "total GPU memory: "
                + str(torch.cuda.get_device_properties(device=args.gpu).total_memory)
            )
            print(
        ...
        path = os.path.join("results", "ae_gan", str(processed_log["id"]))
        ...
        if os.path.isdir(os.path.join("split_logs", log_name)):
            for file_name in sorted(os.listdir(os.path.join("split_logs", log_name))):
                if file_name.startswith("split_log_"):
                    split_log_file_name = os.path.join(
                        "split_logs", log_name, file_name
                    )
        ...
        with open(
            os.path.join(
                path, "split_log_" + dt_object.strftime("%Y%m%d%H%M") + ".json"
            ),
            "w",
        ) as f:
        ...
        attributes_meta = {
            0: {
                "nb_special_tokens": nb_special_tokens,
                "vocabulary_size": log_with_prefixes["vocabulary_size"],
            },
        ...
        model_generator = models.SequentialAutoEncoder(
        ...
        model_discriminator = models.SequentialDiscriminator(
            hidden_size=args.hidden_dim,
            num_layers=args.n_layers,
            dropout_prob=args.dropout_prob,
            vocab_size=attributes_meta[0]["vocabulary_size"],
        ...
        model = (model_generator, model_discriminator)
        ...
        training_log_filename = (
            "training_figures_" + dt_object.strftime("%Y%m%d%H%M") + ".csv"
        )
        ...
        for e in range(args.nb_epoch):
            ...
            training_duration = (
                dt_object_training_end - dt_object_training_start
            ).total_seconds()
            ...
            if args.early_stopping:
                early_stopping_var = 100
                if len(total_validation_losses) > early_stopping_var:
                    if np.all(
                        np.array(total_validation_losses)[-(early_stopping_var - 1) :]
            ...
            if total_validation_loss < min_loss_threshold:
                model_to_save["model_state_dict"] = copy.deepcopy(model[0].state_dict())
                model_to_save["optimizer_state_dict"] = copy.deepcopy(
                    optimizer[0].state_dict()
                )
                model_to_save["loss"] = copy.deepcopy(total_validation_loss)
                model_to_save["epoch"] = copy.deepcopy(e)
                model_to_save["total_validation_loss"] = copy.deepcopy(
                    total_validation_loss
                )
        ...
        if len(model_to_save) > 0:
            checkpoint_name = (
                "model-"
                + "{:.4f}".format(model_to_save["total_validation_loss"])
                + "_epoch-"
                + str(model_to_save["epoch"])
                + "_date-"
                + dt_object.strftime("%Y%m%d%H%M")
                + ".pt"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--datetime",
        help="datetime",
        default=dt_object.strftime("%Y%m%d%H%M"),
        type=str,
    )
    parser.add_argument(
        "--hidden_dim", help="hidden state dimensions", default=128, type=int
    )
    ...
    parser.add_argument(
        "--training_batch_size",
        help="number of training samples in mini-batch",
        default=1152,
        type=int,
    )
    parser.add_argument(
        "--validation_batch_size",
        help="number of validation samples in mini-batch",
        default=1152,
        type=int,
    )
    parser.add_argument(
        "--training_mlm_method", help="training MLM method", default="BERT", type=str
    )
    parser.add_argument(
        "--validation_mlm_method",
        help="validation MLM method",
        default="fix_masks",
        type=str,
    )  # we would like to end up with some non-stochastic & at least pseudo likelihood metric
    parser.add_argument(
        "--mlm_masking_prob", help="mlm_masking_prob", default=0.15, type=float
    )
    parser.add_argument("--dropout_prob", help="dropout_prob", default=0.3, type=float)
    parser.add_argument(
        "--training_learning_rate", help="GD learning rate", default=1e-4, type=float
    )
    parser.add_argument(
        "--training_gaussian_process", help="GP", default=1e-5, type=float
    )
    parser.add_argument(
        "--validation_split", help="validation_split", default=0.2, type=float
    )
    ...
    parser.add_argument(
        "--validation_indexes",
        help="list of validation_indexes NO SPACES BETWEEN ITEMS!",
        default="[0,1,4,10,15]",
        type=str,
    )
    parser.add_argument(
        "--ground_truth_p", help="ground_truth_p", default=0.0, type=float
    )
    parser.add_argument("--architecture", help="BERT or GPT", default="BERT", type=str)
    parser.add_argument(
        "--time_attribute_concatenated",
        help="time_attribute_concatenated",
        default=False,
        type=bool,
    )
    ...
    parser.add_argument(
        "--lagrange_b", help="Langrange multiplier b", default=1.0, type=float
    )
    parser.add_argument(
        "--save_criterion_threshold",
        help="save_criterion_threshold",
        default=4.0,
        type=float,
    )
    parser.add_argument("--pad_token", help="pad_token", default=0, type=int)
    parser.add_argument(
        "--to_wrap_into_torch_dataset",
        help="to_wrap_into_torch_dataset",
        default=True,
        type=bool,
    )
    parser.add_argument(
        "--seq_ae_teacher_forcing_ratio",
        help="seq_ae_teacher_forcing_ratio",
        default=0.7,
        type=float,
    )
    parser.add_argument(
        "--early_stopping", help="early_stopping", default=True, type=bool
    )
    parser.add_argument(
        "--single_position_target",
        help="single_position_target",
        default=False,
        type=bool,
    )
