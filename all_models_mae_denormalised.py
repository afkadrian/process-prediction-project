...
def main(args):
                ...
                for log in logs:
                    if model_type in results.keys():
                        if log in results[model_type].keys():
                            table_row.append('{:.2f}'.format(float(results[model_type][log]['mae_denormalised'])))
    ...
    with open(os.path.join(path, 'table_mae_normalised.tex'), 'w') as tf:
    ...
    for log in logs:
        ...
        twin_subplots[log].yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, pos: '{:.0f}'.format(x / 1000) + 'K'))
    ...
    fig.savefig(os.path.join(path, 'all_models_mae_denormalised.eps'), format='eps')
