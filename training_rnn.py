...
def train_rnn(args, log, log_name, dt_object):
    ...
    with open(
        os.path.join(path, "split_log_" + dt_object.strftime("%Y%m%d%H%M") + ".json"),
        "w",
    ) as f:
    ...
    model = models.SequentialDecoder(
...
def main(args, dt_object):
    ...
    if args.device == "GPU":
        ...
        print(
            "allocated GPU memory: " + str(torch.cuda.memory_allocated(device=args.gpu))
        )
